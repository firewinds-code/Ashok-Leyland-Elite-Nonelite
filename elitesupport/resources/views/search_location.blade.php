						...
						<form name="myForm" id="myForm" method="post" enctype="multipart/form-data" action="{{url('ticket-creation-data')}}" onsubmit="return formSubmit()">
							...
							<div class="row" >
								...
								<div class="form-group col-md-3">
									...
									{{-- <input type="hidden" name="owner_landline" id="owner_landline" class="form-control"  placeholder="Landline Number" /> --}}
			                        <span id="engine_number_error" style="color:red"></span> 
			                    </div>
								<div class="form-group col-md-3">
								...
								<div class="form-group col-md-3">
									...
									<input type="text" name="alse_mail" id="alse_mail" class="form-control"  placeholder="TSM Email" readonly/>
								...
								<div class="form-group col-md-3">
									...
									<input type="text" name="asm_mail" id="asm_mail" class="form-control"  placeholder="RSM Email" readonly/>
							...
							<div class="row" >
								...
								<div class="form-group col-md-3">
									...
									{{-- <input type="hidden" name="owner_contact_mob" id="owner_contact_mob" class="form-control" /> --}}
			                        <span id="engine_number_error" style="color:red"></span> 
			                    </div>
								<div class="form-group col-md-3">
									...
									{{-- <input type="hidden" name="owner_contact_email" id="owner_contact_email" class="form-control" /> --}}
			                    </div>
							</div>
							<div class="row" >
								<div class="container-fluid">
									<div class="col-sm-12 text-center">
										<a class="btn-secondary" id="ownerContactEdit" onclick="ownerContactUpdate(vehicleId.value,ownerId.value,owenerContactId.value,contact_name.value,owner_contact_mob.value,owner_contact_email.value);" style="display: none; color: #fff;padding: 5px;border-radius: 10px;position: relative;top: 10px;">Save Caller</a>
									</div>
								 </div>
							</div>
							<hr>
							...
							<div class="row" >
								<div class="container-fluid">
									<div class="col-sm-12 text-center">
										<a class="btn-secondary" id="callerEdit" onclick="callerUpdate(vehicleId.value,ownerId.value,callerId.value,caller_type.value,caller_name.value,caller_contact.value,caller_language.value);" style="display: none; color: #fff;padding: 5px;border-radius: 10px;position: relative;top: 10px;">Save</a>
							...
							<div class="row">
										...
										<select name="remark_type" id="remark_type" class="form-control" required>
											...
											@isset($remark_type)
												@foreach ($remark_type as $row)
													...
													@if((Auth::user()->role == '29' || Auth::user()->role == '30' || Auth::user()->role == '87') && in_array($row->id,$allowStatus))
									...
									<div class="form-group col-md-3">
										...
										<span style="color: red;">*</span>
										<input type="text" name="estimated_response_time" id="estimated_response_time" autocomplete="off" class="form-control" required >
...
function formSubmit(){
 		...
 		if(vehicleId =='' || vehicleId == 0){
			...
			toastr.info("Please save owner info");
 			return false;
 		}else if(owenerContactId == '' || owenerContactId == 0){
					...
					if(ReultAjax =='Yes'){
						...
						$.ajax({ url: '{{url("get-vehicle-details")}}',
							...
							success: function(result){
								console.log(result);
								if(result =='no'){
									alert("Redirect to Standard Support");
									window.location.href = "https://helpline.ashokleyland.com/non_elitesupport/autologin?id={{base64_encode(Auth::user()->id)}}";
									/* $('#callerEdit').show();
									$('#vehicleEdit').show(); */
	...
	//
		...
		$.ajax({ url: '{{url("get-vehicle-details")}}',
			...
			success: function(data){
				console.log(data);
				if(data =='no'){
					toastr.info("There is no Vehicle");
					$('#callerEdit').show();
					$('#vehicleEdit').show();
					//$('#ownerEdit').show();
...
<script type="text/javascript">
	var lat='';
	function my_function(){ // Without Route
	...
	function callerUpdate(vehicleId,ownerId,callerId,caller_type,caller_name,caller_contact,caller_language)
	{		
		if(caller_language != ''){
			$.ajax({ url: '{{url("caller-update")}}',
				data:{'vehicleId':vehicleId,'ownerId':ownerId,'callerId':callerId,'caller_type':caller_type, 'caller_name':caller_name,'caller_contact':caller_contact,'caller_language':caller_language},
				success: function(response){
					var res =  response.split('~~');
					$('#callerId').val(res[0]);
	...
	{		
		...
		if(owner_name ==''){
			toastr.success("Please select owner");
			$('#owner_name').focus();
		}else if(reg_number1 == ''){
			toastr.info("Please fill registration number");		
		}
		/* else if(chassis_number1 == ''){
			alert("Please fill chassis number");
		}else if(engine_number1 == ''){
			alert("Please fill engine number");
		} */
...
<script>
	...
	function my_function_Route(){ // Route
		var phone = phoneNumber.value;
    ...
    // function manualGoogleMap(lat, long) { // Route Map
    function manualGoogleMap_Route(lat, long) { // Route Map
		if(isLatitude(lat) && isLongitude(long)){
			$('#callerEdit').show();
	...
	async function addMarker_Route(lat, long, el) {  // With  Route
