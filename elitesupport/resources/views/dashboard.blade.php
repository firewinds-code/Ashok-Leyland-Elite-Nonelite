...
<script type="text/javascript">
	...
	function pieChartFunction(datefrom1,dateto1,zone,state,city,dealer,ticketStatus,tat,del){
		...
		$.ajax({  url: '{{url("ajax-ticket-type")}}', 
		data: { 'datefrom1':datefrom1,'dateto1':dateto1,'AllZone':AllZone,'Allstate':Allstate,'Allcity':Allcity,'Alldealer':del,'AllticketStatus':AllticketStatus,'tat':tat,'_token':"{{ csrf_token() }}"},
		...
		type: 'POST',
		data: { _token: CSRF_TOKEN,'datefrom1':datefrom1,'dateto1':dateto1,'AllZone':AllZone,'Allstate':Allstate,'Allcity':Allcity,'Alldealer':del,'AllticketStatus':AllticketStatus,'tat':tat,'_token':"{{ csrf_token() }}"},
...
<script type="text/javascript">
	...
	function fn_zone_change(zoneId,ell){
		var myarray= [];
			...
			$.ajax({ url: '{{url("get-multiple-zone-change")}}',
				data: { 'zoneId':zz,'_token':"{{ csrf_token() }}"},
				success: function(data){
	...
	function Dealer_State_change(el,ell,elll)
	{
		...
		data: { 'r_id':AllZone,'s_id':AllState,'_token':"{{ csrf_token() }}" },
		success: function(data){		
		var Result = data.split(",");var str = '';
		Result.pop();
		for (item in Result)
	...
	function getCityChangeDealer(zone,region,area,dealer){
		...
		data: { 'zone':AllZone,'region':AllState,'city':AllCity,'_token':"{{ csrf_token() }}" },
		success: function(data){
		var Result = data.split(",");var str = '';
