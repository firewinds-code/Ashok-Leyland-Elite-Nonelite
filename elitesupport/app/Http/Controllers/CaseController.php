...
class CaseController  extends Controller{
		   ...
		   /* Followup Tables updates */
			...
			if(Auth::user()->role == 29 || Auth::user()->role == 30 || Auth::user()->role == 87 || Auth::user()->role == 78){
				...
				try {
					...
					DB::table('cases')->where('id', $caseId)->update(['remark_type' => $remark_type,'assign_remarks' => $assign_remarks,'tat_scheduled' => $tat_scheduled,'estimated_response_time' => $estimated_response_time,'restoration_type' => $restoration_type,'response_delay_reason' => $response_delay_reason,'actual_response_time'=>"$actual_response_time",'source' => $source,'followup_time' => $followup_time,'restoration_delay' => $restoration_delay,'updated_at'=>$updated_at,'actual_response_time_customer'=>$actual_response_time_customer,'tat_scheduled_customer'=>$tat_scheduled_customer,'so_number'=>$so_number,'jobcard_number'=>$jobcard_number,'actual_response_time' => "$actual_response_time",'reason_reassign'=>$reason_reassign,'SPOC'=>$SPOC]);
			...
			if($remark_type != "Completed" || $remark_type == "Closed" || ($remark_type == 'Reassigned support' && $db_remark_type != $remark_type)){
				...
				if($assign_work_manager_mobile !=''){
					...
					if($remark_type == 'Reassigned support' && $db_remark_type != $remark_type){
						$message= urlencode('AL Elite customer complaint number '.$complaint_number.' received on '.$currentDateTime.' is reassigned to '.$assign_to.'. Customer Name – '.$db_owner_name.', Caller name- '.$caller_name.', Caller Number- '.$caller_contact.', Vehicle Reg No – '.$reg_number1.', Location- '.$caller_location.', Issue- '.$standard_remark.'. Thank You	Ashok Leyland Elite.');
					}
					// if($remark_type == "Completed" || $remark_type == "Closed"){
					// if($remark_type != "Completed" || $remark_type == "Closed"){
					if($remark_type == "Closed"){
						$message= urlencode('AL Elite customer complaint number '.$complaint_number.' is closed on '.$currentDateTime.'  Response Time -'.$actual_response_time.', Restoration Time -'.$tat_scheduled.', Customer Name - '.$db_owner_name.', Registration Number - '.$reg_number1.'. Thank You Ashok Leyland Elite.');
					}
				...
				// $mobileCust= $assign_work_manager_mobile11.','.$owner_contact_mob.','.$dealer_mob_number.',918105736911';
				$mobileCust= $assign_work_manager_mobile11.','.$owner_contact_mob.','.$dealer_mob_number;
					//dd($mobile);
					//$complaint_number=123;
				// if(($remark_type != "Completed" || $remark_type == "Closed") && $owner_contact_mob !=''){
				...
				if(($remark_type == 'Reassigned support' && $db_remark_type != $remark_type) && $mobileCust !=''){
					...
					//$message= urlencode('Dear AL ELITE Customer, Your complaint number '.$complaint_number.' for vehicle '.$reg_number1.' is reassigned to '.$assignDealerName.' on '.$currentDateTime.'. WM number- '.$assign_work_manager_mobile.'. Regret for the inconvenience.	Thank You Ashok Leyland Elite.');
					try {
			...
			$remType = $remark_type;			
			$sbjct1 ="$remType Mail - AL Elite Ticket Details - $complaint_number | $regNo";
			$sbjct2 ="$remType Mail - AL Elite BSVI Ticket Details - $complaint_number | $regNo";
			...
			if($remark_type == 'Reassigned support'){
				$cloBody1 ='<p>Please find below mentioned the details for the ELITE Support ticket reassigned.</p>';
			}else if($remark_type == 'Closed'){				
				$cloBody1 ='<p>Please find below AL ELITE Support ticket Closure Email.</p>';
			}else{
				$cloBody1 ='Please find below the vehicle status :';
			}
			$subject1="AL ELITE Support Ticket - $complaint_number - $regNo - $remark_type";
			...
			<table border="1" style="font-family: sans-serif;">
				...
				if($remark_type != 'Closed'){
					...
					if($remark_type == "Closed" || ($db_remark_type != $remark_type)){
						try {
			...
			$ccOwnerEmail = array_merge($ccOwnerEmail,$asmOwnerEmailArr);
			$ccOwnerEmail = array_merge($ccOwnerEmail,$ccUserTeamArr);
			$subject1="AL ELITE Support Ticket - $complaint_number - $regNo";
					$body1 = '<p>Dear Customer, </p>
	...
	public function createCaseByApi(Request $request){
		...
		$assign_email = (sizeof($emp_query) !=0)?$emp_query[0]->email:'elitesupport@ashokleyland.com';
		...
		$rep_mnger = (sizeof($emp_query) !=0)?$emp_query[0]->reporting_manager:'';
	 	...
	 	$reporting_manager = (sizeof($rep_mngerSql) >0)?($rep_mngerSql[0]->email):'elitesupport@ashokleyland.com';
		...
		foreach($ccMailIdArray as $row){
			...
			$ccMailId .= (sizeof($getCCMail) !=0)?$getCCMail[0]->email.",":'elitesupport@ashokleyland.com,';
		}
