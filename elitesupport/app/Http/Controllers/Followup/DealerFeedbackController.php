...
class DealerFeedbackController extends Controller
{
    ...
    public function index()
    {
        ...
        try{
            ...
            DB::table('cogent_dealer_followups')
                 ->where('lock_status', '1')
                 ->whereIn('flag', ['0'])
            ...
            /* Followup */
                ...
                ->update(['lock_status' => 0]);
            /* Followup */
             $first = DB::table('cogent_dealer_followups')
                 ->where('lock_status', '0')
                 ->whereNull('first_disposition')
                 ->count();
 
             $followup = DB::table('cogent_dealer_followups')
                 ->where('lock_status', '0')
             ...
             //dd($followup . "=>" . $first);
              ...
              $count['dayCount']=DB::table('cogent_dealer_followups')
                                ->where('lock_status', '0')
                                ->where('shift_time', 'Yes')
                                ->where('flag','!=', 'C')
                                ->count();
              $count['dayFreshCount']=DB::table('cogent_dealer_followups')
                                ->where('lock_status', '0')
                                ->where('flag', '0')
                                ->where('shift_time', 'Yes')
                                // ->whereNull('first_disposition')
              ...
              $count['dayFollowupCount']=DB::table('cogent_dealer_followups')
                                        ->where('lock_status', '0')
                                        ->where('flag', '1')
                                        ->where('shift_time', 'Yes')
                                        // ->whereNotNull('first_disposition')
              ...
              $count['fullDayCount']=DB::table('cogent_dealer_followups')
                                        ->where('lock_status', '0')
                                        ->where('shift_time', 'No')
                                        ->where('flag','!=', 'C')
                                        ->count();
              $count['fullDayFreshCount']=DB::table('cogent_dealer_followups')
                                            ->where('lock_status', '0')
                                            ->where('flag', '0')
                                            ->where('shift_time', 'No')
                                            // ->whereNull('first_disposition')
                                            ->count();
              $count['fullDayFollowupCount']=DB::table('cogent_dealer_followups')
                                                ->where('lock_status', '0')
                                                ->where('flag', '1')
                                                ->where('shift_time', 'No')
                                                // ->whereNotNull('first_disposition')
             ...
             return view('followup.dealer', compact('first', 'followup','count','totalCountAssign','totalCountDealer','totalCountComplete'));
    ...
    public function getLangcount($lang,$shift_time,$type){
        if($type=='Fresh'){
            return $records=DB::table('cogent_dealer_followups')
    ...
    public function dayFresh(Request $request)
    {
        try {
            ...
            //dd($records);
            if(sizeof($records)>0){
                $view = view("followup.ajax.daydealer", compact('type1','type2','records'))->render();
    ...
    public function langAjax(Request $request)
    {
        try {
           ...
           //$view = DB::table('cogent_dealer_followups')->get()->toArray();
           if($request->type2=='Fresh'){
           $view=DB::table('cogent_dealer_followups')
                    ->where('lock_status', '0')
                    ->where('flag', '0')
            ...
            if(sizeof($view)>0){
                ...
                DB::table('chunk_followup_logs')->insert(['complaint_number'=>$cmNumber,'action_by'=>$authId,'type'=>'Dealer Followup']);
    ...
    public function update(Request $request)
    {
        try {
           //dd($request->all());
            ...
            $query= DB::table('cogent_dealer_followups')->select("shift_time")->where("complaint_number",$complaint_number)->get();
        ...
        if($request->type1=='day' && $request->type2=='Fresh'){
            ...
            if($disposition =='Non-Contacted'){
                ...
                if($sub_disposition == 'Status Updated'){
                    $updateArr['flag']='C';
                }else{
                    $updateArr['flag']='1';
                }
            ...
            /* $update =DB::table('cogent_dealer_followups')->where('id',$request->dataid)->update($updateArr);
            //dd($update );
            $notification = array(
            ...
            if($disposition =='Contacted' && $sub_disposition == 'Status Updated'){
                $updateArr['flag']='C';
            }
            /* $update =DB::table('cogent_dealer_followups')->where('id',$request->dataid)->update($updateArr);
            //dd($update );
           ...
           $insertArr = array(
            ...
            DB::table('cogent_dealer_followups_logs')->insert($insertArr);
            //dd($update );
            $notification = array(
                'message' =>"Followup Submited Successfully.",
                'alert-type' => 'success'
