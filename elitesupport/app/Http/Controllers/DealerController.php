...
class DealerController extends Controller
{
	...
	public function asmDealer(){
		try{
			$loginRole = Auth::user()->role;
			$loginCity = Auth::user()->city;
			
			$loginCityArr = explode(",",$loginCity);
			if($loginRole != 78){
				// dd($loginRole);
				$notification = array(
					'message' => "Sorry, you are not autherized",
					'alert-type' => 'error'
				);
				// return back()->with($notification);	
				return redirect()->route('dashboard2')->with($notification);
			}
			...
			$data['rowData']= DB::table('mstr_dealer as d')->select( 'd.id','d.plant_code','d.sac_code','d.dealer_name','d.address','d.state', 'd.city', 'd.zone','d.pincode','d.phone','d.latitude','d.longitude','d.mail','d.working_mon_fri','d.working_sat','d.working_sun','d.working_hours','d.sunday_working','d.dealer_type','r.region','d.SC_State_Name','d.SC_City_Name','d.bsvi','d.area_champion','d.region_champion','d.flag','d.night_spoc_1_name','d.night_spoc_1_number','d.night_spoc_2_name','d.night_spoc_2_number','d.shift_time','s.state as stateName','c.city as cityName')->leftjoin('mstr_region as r','r.id','d.zone')->leftjoin('mstr_state as s','s.id','d.state')->leftjoin('mstr_city as c','c.id','d.city')->whereIn('d.city',$loginCityArr)->orderBy('id','desc')->get();
			
			
			$data['regionData']= DB::table('mstr_region')->select('id','region')->get();
			return view('asm_dealer',$data);
		}catch (\Exception $ex) {
			$notification = array(
                'message' => $ex->getMessage(),
                'alert-type' => 'error'
            );
			return redirect()->route('asm-dealer')->with($notification);
        }
			
	}
	public function rsmDealer(){
		try{
			$loginRole = Auth::user()->role;
			$loginState = Auth::user()->state;
			$loginStateArr = explode(",",$loginState);
			if($loginRole != 79){
				// dd($loginRole);
				$notification = array(
					'message' => "Sorry, you are not autherized",
			...
			$data['rowData']= DB::table('mstr_dealer as d')->select( 'd.id','d.plant_code','d.sac_code','d.dealer_name','d.address','d.state', 'd.city', 'd.zone','d.pincode','d.phone','d.latitude','d.longitude','d.mail','d.working_mon_fri','d.working_sat','d.working_sun','d.working_hours','d.sunday_working','d.dealer_type','r.region','d.SC_State_Name','d.SC_City_Name','d.bsvi','d.area_champion','d.region_champion','d.flag','d.night_spoc_1_name','d.night_spoc_1_number','d.night_spoc_2_name','d.night_spoc_2_number','d.shift_time','s.state as stateName','c.city as cityName')->leftjoin('mstr_region as r','r.id','d.zone')->leftjoin('mstr_state as s','s.id','d.state')->leftjoin('mstr_city as c','c.id','d.city')->whereIn('d.state',$loginStateArr)->orderBy('id','desc')->get();

			
			$data['regionData']= DB::table('mstr_region')->select('id','region')->get();
			return view('rsm_dealer',$data);
		}catch (\Exception $ex) {
			$notification = array(
                'message' => $ex->getMessage(),
                'alert-type' => 'error'
            );
			return redirect()->route('rsm-dealer')->with($notification);
        }
	...
	public function dealer(){
		try{
			...
			$data['rowData']= DB::table('mstr_dealer as d')->select( 'd.id','d.plant_code','d.sac_code','d.dealer_name','d.address','d.state', 'd.city', 'd.zone','d.pincode','d.phone','d.latitude','d.longitude','d.mail','d.working_mon_fri','d.working_sat','d.working_sun','d.working_hours','d.sunday_working','d.dealer_type','r.region','d.SC_State_Name','d.SC_City_Name','d.bsvi','d.area_champion','d.region_champion','d.flag','d.night_spoc_1_name','d.night_spoc_1_number','d.night_spoc_2_name','d.night_spoc_2_number','d.shift_time','s.state as stateName','c.city as cityName')->leftjoin('mstr_region as r','r.id','d.zone')->leftjoin('mstr_state as s','s.id','d.state')->leftjoin('mstr_city as c','c.id','d.city')->orderBy('id','desc')->get();
	...
	public function storeDealer(Request $request){
		try{
			...
			$night_spoc_1_name = $request->input('night_spoc_1_name');
			$night_spoc_1_number = $request->input('night_spoc_1_number');
			$night_spoc_2_name = $request->input('night_spoc_2_name');
			$night_spoc_2_number = $request->input('night_spoc_2_number');
			$shift_time = $request->input('shift_time');
			...
			if(!empty($loginEmail)){
				$loginEmail = str_replace(":",",",$loginEmail); 
			   	$loginEmail = str_replace(";",",",$loginEmail); 
			   	$loginEmail = str_replace(" ","",$loginEmail);
				$loginEmailArr = explode(",",$loginEmail);
			}else{
				$loginEmailArr = array("test@dispostable.com");
			}
			if ($DataID =='') {
				if($loginRole == '78'|| $loginRole == '79'){
					$notification = array(
				...
				$checkDealer = DB::select("Select * from mstr_dealer where sac_code='$sac_code' and dealer_name='$DealerName'");
				...
				DB::table('mstr_dealer')->insert(['dealer_name'=>$DealerName, 'address'=>$address, 'zone'=>$zone, 'state'=>$state, 'city'=>$city, 'pincode'=>$pincode,'phone'=>$phone,'latitude'=>$latitude,'longitude'=>$longitude,'plant_code'=>$plant_code,'sac_code'=>$sac_code,'mail'=>$mail,'working_mon_fri'=>$working_mon_fri,'working_sat'=>$working_sat,'working_sun'=>$working_sun,'working_hours'=>$working_hours,'sunday_working'=>$sunday_working,'dealer_type'=>$dealer_type,'SC_State_Name'=>$SC_State_Name,'SC_City_Name'=>$SC_City_Name,'bsvi'=>$bsvi,'area_champion'=>$area_champion,'region_champion'=>$region_champion,'flag'=>$flag,'night_spoc_1_name'=>$night_spoc_1_name,'night_spoc_1_number'=>$night_spoc_1_number,'night_spoc_2_name'=>$night_spoc_2_name,'night_spoc_2_number'=>$night_spoc_2_number,'shift_time'=>$shift_time]);
				$lastInsertId = DB::getPdo()->lastInsertId();
				//dd($lastInsertId );
				$userDataQuery = DB::select("update users set dealer_id=concat(dealer_id,',',$lastInsertId) where user_type_id=1;");
				/* Add dealer in ZSM,RSM,ASM in Elite*/
				DB::select("update users set dealer_id=concat(dealer_id,',',$lastInsertId) where find_in_set($zone,zone) and role='80' and flag=1");
				DB::select("update users set dealer_id=concat(dealer_id,',',$lastInsertId) where find_in_set($state,state) and role='79' and flag=1");
				DB::select("update users set dealer_id=concat(dealer_id,',',$lastInsertId) where find_in_set($city,city) and role='78' and flag=1");
				/* Add dealer in ZSM,RSM,ASM in Elite*/

				DB::table('non_elitesupport.mstr_dealer')->insert(['dealer_name'=>$DealerName, 'address'=>$address, 'zone'=>$zone, 'state'=>$state, 'city'=>$city, 'pincode'=>$pincode,'phone'=>$phone,'latitude'=>$latitude,'longitude'=>$longitude,'plant_code'=>$plant_code,'sac_code'=>$sac_code,'mail'=>$mail,'working_mon_fri'=>$working_mon_fri,'working_sat'=>$working_sat,'working_sun'=>$working_sun,'working_hours'=>$working_hours,'sunday_working'=>$sunday_working,'dealer_type'=>$dealer_type,'SC_State_Name'=>$SC_State_Name,'SC_City_Name'=>$SC_City_Name,'bsvi'=>$bsvi,'area_champion'=>$area_champion,'region_champion'=>$region_champion,'flag'=>$flag,'night_spoc_1_name'=>$night_spoc_1_name,'night_spoc_1_number'=>$night_spoc_1_number,'night_spoc_2_name'=>$night_spoc_2_name,'night_spoc_2_number'=>$night_spoc_2_number,'shift_time'=>$shift_time]);
				$lastInsertId1 = DB::getPdo()->lastInsertId();
				//dd($lastInsertId );
				$userDataQuery = DB::select("update non_elitesupport.users set dealer_id=concat(dealer_id,',',$lastInsertId1) where user_type_id=1;");
				/* Add dealer in ZSM,RSM,ASM in Standard*/
				DB::select("update non_elitesupport.users set dealer_id=concat(dealer_id,',',$lastInsertId) where find_in_set($zone,zone) and role='80' and flag=1");
				DB::select("update non_elitesupport.users set dealer_id=concat(dealer_id,',',$lastInsertId) where find_in_set($state,state) and role='79' and flag=1");
				DB::select("update non_elitesupport.users set dealer_id=concat(dealer_id,',',$lastInsertId) where find_in_set($city,city) and role='78' and flag=1");
				/* Add dealer in ZSM,RSM,ASM in Standard*/
				...
				if($loginRole == '78' || $loginRole == '79'){
					DB::table('mstr_dealer')->where('id', $DataID)->update(['night_spoc_1_name'=>$night_spoc_1_name,'night_spoc_1_number'=>$night_spoc_1_number,'night_spoc_2_name'=>$night_spoc_2_name,'night_spoc_2_number'=>$night_spoc_2_number,'working_mon_fri'=>$working_mon_fri,'sunday_working'=>$sunday_working,'dealer_type'=>$dealer_type]);
					
					DB::table('non_elitesupport.mstr_dealer')->where('id', $DataID)->update(['night_spoc_1_name'=>$night_spoc_1_name,'night_spoc_1_number'=>$night_spoc_1_number,'night_spoc_2_name'=>$night_spoc_2_name,'night_spoc_2_number'=>$night_spoc_2_number,'working_mon_fri'=>$working_mon_fri,'sunday_working'=>$sunday_working,'dealer_type'=>$dealer_type]);

					try {
						$body = '<p>Dear Team, </p>
						<p>SAC Code: '.$sac_code.' Dealer has been updated By '.Auth::user()->name.'.</p>
						<p>Below mentioned changes has been done</p>
						<table border="1" style="font-family: sans-serif;">
							<tr>
								<td style=" text-align: left;">Dealer Name</td>
								<td style="text-align: left;">'.$DealerName.'</td>
							</tr>
							<tr>
								<td style=" text-align: left;">SAC Code</td>
								<td style="text-align: left;">'.$sac_code.'</td>
							</tr>
							<tr>
								<td style=" text-align: left;">Night SPOC 1 Name</td>
								<td style="text-align: left;">'.$night_spoc_1_name.'</td>
							</tr>
							<tr>
								<td style=" text-align: left;">Night SPOC 1 Number</td>
								<td style="text-align: left;">'.$night_spoc_1_number.'</td>
							</tr>
							<tr>
								<td style=" text-align: left;">Night SPOC 2 Name</td>
								<td style="text-align: left;">'.$night_spoc_2_name.'</td>
							</tr>
							<tr>
								<td style=" text-align: left;">Night SPOC 2 Number</td>
								<td style="text-align: left;">'.$night_spoc_2_number.'</td>
							</tr>
						</table>			
						<p>Regards,</p>
						<p>ELITE Support Cell</p>';
						$subject = 'Dealer Update';

						// 'kuldeep.Sharma@ashokleyland.com','Suresh.J@ashokleyland.com'
						$toUserArr1 = array('kuldeep.Sharma@ashokleyland.com','KRYSALIS_Vandhana1@ashokleyland.com','ramzin.sulthan@cogenteservices.com');
						$toUserArr = array_merge($toUserArr1, $loginEmailArr);
						$toUserArr = array_filter($toUserArr);
						// $ccUserArr = array('ashutosh.rawat@cogenteservices.in','ramzin.sulthan@cogenteservices.com');
			
			
						$data=['body'=>$body];
						Mail::send('assigned_email',["data"=>$data],function ($message) use ($toUserArr, $subject) {
							$message->to($toUserArr)->bcc(['ramzin.sulthan@cogenteservices.com'])->subject($subject);
							$message->from('elitesupport@ashokleyland.com');
						});
						
					} catch (\Exception $ii){
						$ii  = $ii->getMessage();
						DB::table('updation_exception')->insert(['complaint_number'=>"ASM Dealer",'type'=>"Dealer Update by ASM",'exception'=>"$ii"]);
				   }

				}else{
					DB::table('mstr_dealer')->where('id', $DataID)->update(['dealer_name'=>$DealerName, 'address'=>$address, 'zone'=>$zone, 'state'=>$state, 'city'=>$city, 'pincode'=>$pincode,'phone'=>$phone,'latitude'=>$latitude,'longitude'=>$longitude,'plant_code'=>$plant_code,'sac_code'=>$sac_code,'mail'=>$mail,'working_mon_fri'=>$working_mon_fri,'working_sat'=>$working_sat,'working_sun'=>$working_sun,'working_hours'=>$working_hours,'sunday_working'=>$sunday_working,'dealer_type'=>$dealer_type,'updated_at'=>$updated_at,'SC_State_Name'=>$SC_State_Name,'SC_City_Name'=>$SC_City_Name,'bsvi'=>$bsvi,'area_champion'=>$area_champion,'region_champion'=>$region_champion,'flag'=>$flag,'night_spoc_1_name'=>$night_spoc_1_name,'night_spoc_1_number'=>$night_spoc_1_number,'night_spoc_2_name'=>$night_spoc_2_name,'night_spoc_2_number'=>$night_spoc_2_number,'shift_time'=>$shift_time]);
				
					DB::table('non_elitesupport.mstr_dealer')->where('id', $DataID)->update(['dealer_name'=>$DealerName, 'address'=>$address, 'zone'=>$zone, 'state'=>$state, 'city'=>$city, 'pincode'=>$pincode,'phone'=>$phone,'latitude'=>$latitude,'longitude'=>$longitude,'plant_code'=>$plant_code,'sac_code'=>$sac_code,'mail'=>$mail,'working_mon_fri'=>$working_mon_fri,'working_sat'=>$working_sat,'working_sun'=>$working_sun,'working_hours'=>$working_hours,'sunday_working'=>$sunday_working,'dealer_type'=>$dealer_type,'updated_at'=>$updated_at,'SC_State_Name'=>$SC_State_Name,'SC_City_Name'=>$SC_City_Name,'bsvi'=>$bsvi,'area_champion'=>$area_champion,'region_champion'=>$region_champion,'flag'=>$flag,'night_spoc_1_name'=>$night_spoc_1_name,'night_spoc_1_number'=>$night_spoc_1_number,'night_spoc_2_name'=>$night_spoc_2_name,'night_spoc_2_number'=>$night_spoc_2_number,'shift_time'=>$shift_time]);
				}
	...
	public function dealerDelete($id){
		try{
			...
			DB::table('mstr_dealer')->where('id', $id)->delete();
			DB::table('non_elitesupport.mstr_dealer')->where('id', $id)->delete();
			$notification = array(
	...
	public function cityChangeGetDealer(Request $request){
		...
		$result=DB::select("select id,dealer_name,sac_code from mstr_dealer where find_in_set(zone,'".$zone."') and find_in_set(state,'".$State."') and find_in_set(city,'".$city."' ) and flag=1 order by dealer_name ASC");
		foreach($result as $value){
