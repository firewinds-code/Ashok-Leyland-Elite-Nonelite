...
/* ----------------------Users---------------------------------  */	
	public function asmUser(Request $request){ 
		try{

			$loginRole = Auth::user()->role;
			$loginZone = Auth::user()->zone;
			...
			$data['regionData']= DB::table('mstr_region')->select('id','region')->where('id',$loginZone)->orderBy('region','ASC')->get();
			
			$data['userData']= DB::table('users')->select('id','name','employee_id')->get();
			//$data = DB::table('users')->select('id','name','email')->get();
			$k=0;
			foreach($loginCityArr as $asmCity){		
				$k++;		
				$dataAjax123[] = DB::table('users')->select('users.id as id','users.employee_id as employee_id','users.assign_complaint as assign_complaint','users.dealer_id as dealer_id', 'users.name as name', 'users.last_name as last_name', 'users.email', 'users.user_type_id as user_Type_id', 'users.role as role', 'users.state as state', 'users.city as city', 'users.zone as zone', 'users.mobile as mobile', 'users.flag as flag','mstr_user_type.usertype as usertype','mstr_dealer.sac_code','mstr_dealer.dealer_name as dealer_name','mstr_role.role as role_name')->leftjoin('mstr_user_type','mstr_user_type.id','=','users.user_type_id')->leftjoin('mstr_dealer','mstr_dealer.id','=','users.dealer_id')->leftjoin('mstr_role','mstr_role.id','=','users.role')->where('users.flag','1')->whereRaw('FIND_IN_SET(?, users.city)', [$asmCity])->where('users.id','!=','2599')->whereIn('users.user_type_id',array(2,3,4))->where('users.role','!=','31')->get();
				
			}
			$data['rowData'] = collect($dataAjax123)->flatten(1);			
			return view('asm_user',$data);
		}catch (\Exception $ex) {
			$notification = array(
				'message' => $ex->getMessage(),
				'alert-type' => 'error'
	...
	public function rsmUser(Request $request){ 
		try{
			...
			//$data = DB::table('users')->select('id','name','email')->get();
			$k=0;
			foreach($loginStateArr as $row){		
				$k++;		
				$dataAjax123[] = DB::table('users')->select('users.id as id','users.employee_id as employee_id','users.assign_complaint as assign_complaint','users.dealer_id as dealer_id', 'users.name as name', 'users.last_name as last_name', 'users.email', 'users.user_type_id as user_Type_id', 'users.role as role', 'users.state as state', 'users.city as city', 'users.zone as zone', 'users.mobile as mobile', 'users.flag as flag','mstr_user_type.usertype as usertype','mstr_dealer.sac_code','mstr_dealer.dealer_name as dealer_name','mstr_role.role as role_name')->leftjoin('mstr_user_type','mstr_user_type.id','=','users.user_type_id')->leftjoin('mstr_dealer','mstr_dealer.id','=','users.dealer_id')->leftjoin('mstr_role','mstr_role.id','=','users.role')->where('users.flag','1')->whereRaw('FIND_IN_SET(?, users.state)', [$row])->where('users.id','!=','2599')->whereIn('users.user_type_id',array(2,3,4))->where('users.role','!=','31')->get();
				
			}
			
			$data['rowData'] = collect($dataAjax123)->flatten(1);
						
			return view('rsm_user',$data);
		}catch (\Exception $ex) {
			$notification = array(
				'message' => $ex->getMessage(),
				'alert-type' => 'error'
			...
			if ($request->ajax()) {
				...
				return Datatables::of($dataAjax)->addIndexColumn()
					->addColumn('action', function($row){
						...
						<a href="'.route('users_delete.usersDelete', ["id" => $row->id]).'"'.'onclick="return confirm('."'Do you want to delete?'".')" <i class="fa fa-trash-o" aria-hidden="true" style="cursor: pointer;"></i></a>';
						return $btn;
					})		
	...
	public function storeUsers(Request $request){
		...
		if($usertype_id =='3'){
			...
			$dealerQuery = DB::table('mstr_dealer')->select('zone','state','city')->whereIn('id',$dealer_id)->get()->toArray();
			/* $stateArr .= $dealerQuery[0]->state.',';
			$cityArr .= $dealerQuery[0]->city.',';
			$zoneArr .= $dealerQuery[0]->zone.',';	 */
			foreach ($dealerQuery as $value) {
				$stateArr .= $value->state.',';
				$cityArr .= $value->city.',';
				$zoneArr .= $value->zone.',';	
			}
		}
		$stateArr = implode(",",array_unique(explode(",",rtrim($stateArr,','))));
		$cityArr = implode(",",array_unique(explode(",",rtrim($cityArr,','))));
		$zoneArr = implode(",",array_unique(explode(",",rtrim($zoneArr,','))));
		...
		if($dataid == ''){
			...
			/* if($rowData == 0){
				...
				/* Elite Insert */
				/* Non-Elite Insert */
				DB::table('non_elitesupport.users')->insert(['employee_id'=>"$employee_id",'name'=>"$name",'last_name'=>"$last_name",'email'=>"$email",'password'=>"$password",'mobile'=>"$mobile",'user_type_id'=>"$usertype_id",'state'=>"$state",'role'=>"$role",'city'=>"$city" ,'zone'=>"$zone",'dealer_id'=>"$delId",'flag'=>"$flag"]); 
				/* Non-Elite Insert */
				$notification = array(
			...
			DB::table('non_elitesupport.users')->where('id', $dataid)->update(['employee_id'=>"$employee_id",'name'=>"$name",'last_name'=>"$last_name",'email'=>"$email",'mobile'=>"$mobile",'state'=>"$state",'role'=>"$role",'city'=>"$city",'zone'=>"$zone",'dealer_id'=>"$delId",'flag'=>"$flag",'updated_at'=>"$updated_at"]);
			/* Non-ELite */
			$notification = array(
			...
			try {
				$body = '<p>Dear Team, </p>
				<p>Employee Id: '.$employee_id.' User has been updated By '.Auth::user()->name.'.</p>
				...
				$ii  = $ii->getMessage();
				DB::table('updation_exception')->insert(['complaint_number'=>"User Update",'type'=>"User Update",'exception'=>"$ii"]);
		   }
			if($loginRole == '78'){
				return redirect()->route('asm-user')->with($notification);	
			}else if($loginRole == '79'){
				return redirect()->route('rsm-user')->with($notification);
			}else{
				return redirect()->route('users')->with($notification);	
			}
		}
	...
	public function usersDelete($id){
		//DB::table('users')->where('id', $id)->delete();
			...
			DB::table('users')->where('id', $id)->delete();
			DB::table('non_elitesupport.users')->where('id', $id)->delete();
			$notification = array(
		...
		try{
			...
			if(Hash::check($current_pwd,$db_password)){
				if($new_password == $confirm_password){
					...
					DB::table("non_elitesupport.users")->where('employee_id','=',$employee_id )->update(['password'=>$new_pwd]);
					$notification = array(
...
/* ----------------------Role---------------------------------  */
	...
	public function storeRole(Request $request){
		...
		if ($usertype_id == 'NA' || $serverValidation->is_empty($role)) {
			...
			DB::table('non_elitesupport.mstr_role')->insert(['usertype_id'=>$usertype_id, 'role'=>$role, 'complaint_type'=>$complaint_type]);
			$notification = array(
				'message' => 'Stored successfully',
				'alert-type' => 'success'
			);
	...
	public function updateRole(Request $request){
		...
		if ($usertype_id =='NA' || $serverValidation->is_empty($role)) {
			...
			$notification = array(
				...
				DB::table('non_elitesupport.mstr_role')->where('id', $id)->update(['usertype_id' => $usertype_id, 'role' => $role, 'flag' => $flag,'updated_at'=>$updated_at]);
				$notification = array(
	                'message' => 'Updated successfully',
	                'alert-type' => 'success'
	            ); 
	...
	public function roleDelete($id){
		...
		DB::table('mstr_role')->where('id', $id)->delete();
		DB::table('non_elitesupport.mstr_role')->where('id', $id)->delete();
		$notification = array(
		        'message' => $delData[0]->Message,
		        'alert-type' => $delData[0]->Action
		);
	...
	public function storeChangePassword(Request $request){
		try {
			...
			if(sizeof($checkUser)>0){
				...
				DB::table('non_elitesupport.users')->where('employee_id', $employee_id)->update(['password' => $pwd,'updated_at'=>$updated_at]);
				$notification = array(
