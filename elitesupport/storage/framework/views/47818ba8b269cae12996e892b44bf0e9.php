...
<script type="text/javascript">
	...
	function pieChartFunction(datefrom1,dateto1,zone,state,city,dealer,ticketStatus,tat,del){
		...
		$.ajax({  url: '<?php echo e(url("ajax-ticket-type")); ?>', 
		data: { 'datefrom1':datefrom1,'dateto1':dateto1,'AllZone':AllZone,'Allstate':Allstate,'Allcity':Allcity,'Alldealer':del,'AllticketStatus':AllticketStatus,'tat':tat,'_token':"<?php echo e(csrf_token()); ?>"},
		...
		$.ajax({  url: '<?php echo e(url("ajax-complaint-search")); ?>',
		type: 'POST',
		data: { _token: CSRF_TOKEN,'datefrom1':datefrom1,'dateto1':dateto1,'AllZone':AllZone,'Allstate':Allstate,'Allcity':Allcity,'Alldealer':del,'AllticketStatus':AllticketStatus,'tat':tat,'_token':"<?php echo e(csrf_token()); ?>"},
		...
		$.ajax({  url: '<?php echo e(url("ajax-pie-search")); ?>',
		data: { 'datefrom1':datefrom1,'dateto1':dateto1,'AllZone':AllZone,'Allstate':Allstate,'Allcity':Allcity,'Alldealer':del,'AllticketStatus':AllticketStatus,'tat':tat,'_token':"<?php echo e(csrf_token()); ?>"},
			beforeSend: function() {
				$('#ajaxLoader').show();
			},
			success: function(data1){
				...
				for (item in result) {
					...
					var options = {title:'Ticket Chart',
					legend:'left',
					pieHole: 0.4,
					width:'100%',
					titleFontSize:14,
					height:'100%','is3D':false,pieSliceText: "value", tooltip: {
						text: 'value'
					}};
		...
		<div class="card-body" style="background: #e5e5e5;">			
			<div class="row">	
				...
				<div class="col-lg-2 box" style="background: #fff;">
					<div class="box-inner">
					<a class="nav-link" href=""><p style="background: #111;color: #fff;" class="sidenav_p">Dashboard</p></a>
					
					<p class="sub_p">
						Date From
						<input type="text" name="datefrom" id="datefrom1" autocomplete="off" class="form-control"  placeholder="Date From" onchange="pieChartFunction(this.value,dateto1.value,zone.value,state.value,city.value,dealer.value,ticketStatus.value,tat.value,'')"/>
					</p>
					<p class="sub_p">
						Date To
						<input type="text" name="dateto" id="dateto1" autocomplete="off" class="form-control"  placeholder="Date To" onchange="pieChartFunction(datefrom1.value,this.value,zone.value,state.value,city.value,dealer.value,ticketStatus.value,tat.value,'')"/>
					</p>
					<p class="sub_p">
						Zone
						<span style="float: right;"></span>
						<div>
							<select name="zone[]" multiple id="zone" class="form-control" onchange="fn_zone_change(this.value,''),pieChartFunction(datefrom1.value,dateto1.value,this.value,state.value,city.value,dealer.value,ticketStatus.value,tat.value,'')" required>
					...
					<p class="sub_p">
						...
						<span style="float: right;"></span>
						<div>
							<select name="state[]" multiple id="state" class="form-control" onchange="Dealer_State_change(zone.value,this.value,''),pieChartFunction(datefrom1.value,dateto1.value,zone.value,this.value,city.value,dealer.value,ticketStatus.value,tat.value,'')" required></select>
						</div>
					</p>
					<p class="sub_p">
						Area
						<span style="float: right;"></span>
						<select name="city[]" multiple id="city" class="form-control" onchange="getCityChangeDealer(zone.value,state.value,this.value,''),pieChartFunction(datefrom1.value,dateto1.value,zone.value,state.value,this.value,dealer.value,ticketStatus.value,tat.value,'')" required></select>
					</p>
					<p class="sub_p">
						Dealer
						<span style="float: right;"></span>
						<div>
							<select name="dealer[]" multiple id="dealer" class="form-control" onchange="pieChartFunction(datefrom1.value,dateto1.value,zone.value,state.value,city.value,dealer.value,ticketStatus.value,tat.value,'')"></select>
						</div>
					</p>
					...
					<p class="sub_p">
						Ticket Status
 						<div>
							...
							<select name="ticketStatus[]" multiple id="ticketStatus" class="form-control" onchange="pieChartFunction(datefrom1.value,dateto1.value,zone.value,state.value,city.value,dealer.value,this.value,tat.value,'')">
					...
					<p class="sub_p">
						...
						<div>
							<select name="tat" id="tat" class="form-control" style="width: 100%" onchange="pieChartFunction(datefrom1.value,dateto1.value,zone.value,state.value,city.value,dealer.value,ticketStatus.value,this.value,'')">
...
<script type="text/javascript">
	...
	function fn_zone_change(zoneId,ell){
		var myarray= [];
			...
			$.ajax({ url: '<?php echo e(url("get-multiple-zone-change")); ?>',
				data: { 'zoneId':zz,'_token':"<?php echo e(csrf_token()); ?>"},
				success: function(data){
	...
	function Dealer_State_change(el,ell,elll)
	{
		...
		data: { 'r_id':AllZone,'s_id':AllState,'_token':"<?php echo e(csrf_token()); ?>" },
		success: function(data){		
		var Result = data.split(",");var str = '';
		Result.pop();
		for (item in Result)
	...
	function getCityChangeDealer(zone,region,area,dealer){
		...
		data: { 'zone':AllZone,'region':AllState,'city':AllCity,'_token':"<?php echo e(csrf_token()); ?>" },
		success: function(data){
		var Result = data.split(",");var str = '';
...
<?php echo $__env->make("layouts.masterlayout", \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\wamp64\www\ashokleyland\elitesupport\resources\views/dashboard.blade.php ENDPATH**/ ?>