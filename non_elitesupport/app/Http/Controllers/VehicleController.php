...
class VehicleController extends Controller
{
	...
	{
		$rowData = DB::select("select o.owner_name,v.reg_number, v.chassis_number, v.engine_number,v.purchase_date, v.add_blue_use, v.engine_emmission_type,v.created_at, v.updated_at, v.flag from mstr_vehicle as v left join mstr_owner as o on o.id = v.ownerId order by v.id desc");

		$columns = array('Owner_Name', 'Registration_Number', 'Chassis_Number', 'Engine_Number', 'purchase_date', 'add_blue_use', 'engine_emmission_type', 'Created_Date', 'Updated_Date', 'Status');

		$fileName = 'Vehicle_Master_."' . date("Y-m-d h:i:s") . '".csv';
		$headers = array(
	...
	public function checkEliteReg(Request $request)
	{
		try {
			...
			$query = DB::select("Select count(*) as cnt from elitesupport.mstr_vehicle v left join elitesupport.mstr_owner o on v.ownerId = o.id and o.flag=1 
			where (o.owner_cat='SUPREME' or o.owner_cat='GRAND') and (v.reg_number=:reg_number or v.chassis_number = :chassis_number or v.engine_number=:engine_number) and v.flag=1", ["reg_number" => $reg_number, "chassis_number" => $chassis_number, "engine_number" => $engine_number]);
			if ($query[0]->cnt == 1) {
				echo "Yes";
				die;
			} else {
