...
class LocationController extends Controller
{
	...
	public function sendLatlongLink(Request $request){
		try{
			...
			Welcome to AL Break Down Support.
			Please click '.$link.' to share your current location to reach you.
			Thank You
			Team Ashok Leyland'); */
			/* $message= urlencode('Dear Customer,Welcome to AL Break Down Support. Please click '.$link.' to share your current location to reach you. Thank You Team Ashok Leyland'); */

			/*  $message= urlencode('Dear Customer,Welcome to AL Break Down Support. Please click https://alpertestcallcenter.ashokleyland.com/non_elitesupport/search-location-mob/'.$phnSess.' to share your current location to reach you. Thank You Team Ashok Leyland'); */
	...
	public function callerUpdate(Request $request){
		try { 
			...
			$caller_contact_alt  = $request->input('caller_contact_alt');
			$caller_language  = $request->input('caller_language');
			DB::table('mstr_caller')->insert(['owner_id'=>$ownerId,'caller_type'=>$caller_type,'caller_name'=>$caller_name,'caller_contact'=>$caller_contact,'caller_contact_alt'=>$caller_contact_alt,'caller_language'=>$caller_language]);
	...
	public function vehicleUpdate(Request $request)
	{
		try {			
			...
			if($purchase_date !=''){
				...
				DB::select("Insert into mstr_vehicle (ownerId,vehicle_model, reg_number, chassis_number, engine_number, vehicle_segment, add_blue_use, engine_emmission_type) values ('$owner_id','$vehicle_model','$reg_number1','$chassis_number1','$engine_number1','$vehicle_segment','$add_blue_use','$engine_emmission_type')");
			}
			$id = DB::getPdo()->lastInsertId();
	...
	public function ownerUpdate(Request $request)
	{
		try {
			...
			DB::select("Insert into mstr_owner (owner_name, owner_mob, owner_landline, owner_cat, owner_company,alse_mail,asm_mail) values ('$owner_name','$owner_mob','$owner_landline','$owner_cat','$owner_company','KRYSALIS_Vandhana1@ashokleyland.com','KRYSALIS_Vandhana1@ashokleyland.com')");
			$id = DB::getPdo()->lastInsertId();
	...
	public function ticketCreationData(Request $request)
	{
		...
		if($checkVehiclleTicket[0]->cnt > 0 ){
			...
			$vahan_status = $request->input('vahan_status');
			$vehicleId = $request->input('vehicleId');
			$ownerId = $request->input('ownerId');
			...
			if($caller_contact !=''){  
				$followup_name = $caller_name.' (Owner)';
				$followups_number = $caller_contact;
			}
			/* *******BS6******* */
			try{
			...
			/* ******************* API ************************** */
					...
					try {						
						...
						$ffq = addslashes($ff->getMessage());
						$ffq ="Error";// str_replace("'","_",$ffq);
						DB::table('creation_exception')->insert(['complaint_number'=>"$complaint_number",'type'=>"msu_api Create",'exception'=>"$ffq"]);
					}
					...
					$gg = $gg->getMessage();
					DB::table('creation_exception')->insert(['complaint_number'=>"$complaint_number",'type'=>"call MSU",'exception'=>"$gg"]);
				}
				/* Call Update MSU API */
				...
				try {
					...
					if($isJsonUpdate){				
						...
						DB::select("INSERT INTO updation_api_remarks (complaint_number, remarks) VALUES ('$complaint_number', '$jsonData')");
					}else{
					...
					DB::table('updation_exception')->insert(['complaint_number'=>"$complaint_number",'type'=>"MSU API Updation creation",'exception'=>"$llMSU"]);
				}	
				/* Call Update MSU API */
				try {
			...
			if($assign_work_manager_mobileCheck !=''){
				...
				$message= urlencode('Dear Service team, Customer complaint number '.$complaint_number.' received on '.$caseCreatedDate.' is assigned to '.$dealerName.'. Company Name – '.$owner_nameMSU.', Caller name- '.$caller_name.', Number- '.$caller_contact.', RegNo – '.$reg_number1.' Location- '.$location.', Issue- '.$standard_remark.' Thank You Ashok Leyland');
			...
			if($caller_contact !=''){
				...
				$owner_mob = '91'.$owner_mob;
				// $mobile='91'.$caller_contact.','.$owner_mob.',918105736911';
				$mobile='91'.$caller_contact.','.$owner_mob;
				...
				Thank You
				Ashok Leyland.'; */
				// $message= urlencode('Dear Customer, Your breakdown complaint number '.$complaint_number.' for vehicle '.$reg_number1.' is assigned to '.$dealerName.'. WM number- '.$assign_work_manager_mobileCheck.'. Thank You Ashok Leyland.');
				...
				try {
					$tsmQuery =DB::select("select mobile from users where find_in_set($assign_to,dealer_id) and role = 1 and flag=1 limit 1");										
						if(sizeof($tsmQuery)>0){
							...
							$this->sendSMSFuction($mobile,$message,'1607100000000289246',"Creation SMS to TSM",$complaint_number);
						}
		...
		//    $owner_contact_email[] = $request->input('owner_contact_email')!=''?$request->input('owner_contact_email'):'KRYSALIS_Vandhana1@ashokleyland.com';
		   $owner_contact_email[] = 'KRYSALIS_Vandhana1@ashokleyland.com';
		   ...
		   /* ********************* Owner ALSE/ASM/RSM******************** */										
			...
			$sbjct1 ="Fresh Mail:AL Breakdown Ticket Details $complaint_number | $reg_number1 - $remark_type";
			$sbjct2 ="Fresh Mail:AL Breakdown BSVI Ticket Details $complaint_number | $reg_number1 - $remark_type";
			...
			/* *********************************************************************************************/
			$subject1="AL Breakdown Ticket Details - $complaint_number | $reg_number1 ";
					$body1 = '<p>Dear Customer, </p>
					<p>Please find below mentioned the details for the AL Breakdown Ticket Details raised.</p>
					<table border="1" style="font-family: sans-serif;">
						<tr>
							<td style="text-align: left;">Customer Name</td>
							<td style="text-align: left;">'.$owner_nameMSU.'</td>
				...
				<p>Regards,</p>
					...
					$kk  = $kk->getMessage();
					DB::table('creation_exception')->insert(['complaint_number'=>"$complaint_number",'type'=>"Customer Mail",'exception'=>"$kk"]);
				} */
			/* Customer Email Send Fields */ 
			/* Cogent Followup */
			try {
	...
	public function mailFunction(){ 
		try {
			...
			/* Mail::send('assigned_email', ["data"=>$data], function ($message) {
				$message->from('ALHelpline@ashokleyland.com', 'ELITE Support');
				$message->sender('ALHelpline@ashokleyland.com', 'ELITE Support');
				$message->to('', '');
			...
			$subject = 'Test Mail';
			$toUserArr = 'bachan.rawat@cogenteservices.com'.','.'shashi.ranjan@cogenteservices.com';
			$toUserArr = explode(',',$toUserArr);
			$ccUserArr = $toUserArr;
			Mail::send('assigned_email',["data"=>$data],function ($message) use ($toUserArr, $ccUserArr, $subject) {
	...
	public function smsCheck(){
		...
		/* $message= urlencode('Dear Customer,Welcome to AL Break Down Support. Please click vsvsdrfvsrdfrs to share your current location to reach you. Thank You Team Ashok Leyland'); */
		$message= urlencode('Dear Service team, Customer complaint number dwdwdwdw received on dww is assigned to dwdwdwd Company Name – ddewdw, Caller name- dedede, Number- deded, RegNo – sdwdwd Location- dwdwdw, Issue- dwdedwed Thank You Ashok Leyland');
				//$message=urlencode($message);
	...
	public function testMSU(){
		...
		CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
		CURLOPT_CUSTOMREQUEST => 'POST',
		CURLOPT_POSTFIELDS =>'{ticketCreatedTime:2023-09-23 10:43:53,ticket_no:STDB232496540,reg_no:CG07CJ1931,chasi_no:MB1A3GCD8NRGY8914,eng_no:NGHZ417978,vehicle_model:CA1615/47 H FBL,vehicle_seg:6,long_hal:Long Haulage,lattitude:22.76237346574333,longitude:84.16557371469295,purchase_date:2022-08-09 00:00:00,eng_emi_type:BS6,owner_name:ABHAY PANDAY,owner_mobile:9179858797,owner_lno:NA,owner_company:ABHAY PANDAY,owner_category:NA,email:KRYSALIS_Vandhana1@ashokleyland.com,asm_email:KRYSALIS_Vandhana1@ashokleyland.com,caller_info:Driver/Caller Info,caller_type:Owner,caller_name:ABHAY PANDAY,caller_no:9179858797,vehicle_from:NA,vehicle_to:NA,bd_location:loro,bd_lm:jalpani,bd_state:Chhattisgarh,bd_district:NA,bd_city:NA,bd_hway:NA,vehicle_type:Warranty,vehicle_condition:No,ticket_type:Breakdown Ticket,vehicle_problem:,dealer_name:Shri Balaji Motor - Chattisgarh,dealer_wmname:Pankaj Meshram,dealer_wmmobileno:6232033936,dealer_workstatus:Assigned,delaer_remark:owner called from 9179858797 he said vehicle has gear issue/ clutch issue and it s not in moving condition so as per process ticket has been assigned to nearest workshop manual location,addBlueUse:Yes,aggregate:Clutch,dealercode:92680,disposition:NA,agentRemarks:NA,acceptance:0,standardRemark:gear issue/ clutch issue,contact_person_mail:NA,contact_person_name:NA,contact_person_phone:NA,ticket_status:Assigned,estimated_response_time:2023-09-23 14:42:45,source:NA,followup_time:2023-09-23 10:57:51}
		',
		CURLOPT_HTTPHEADER => array(
			'Authorization: Basic bXN1Y29nZW50OmlZbVBSaDJubXA=',
			'Content-Type: application/json'
	...
	public function getVehicleDetails(Request $request){
		try{
			...
			DB::select("set sql_mode =''");
			if($reg_number !='000000NA'){
				//$chechVehSql = DB::select("Select v.id as vehicleId, v.vehicle_model, v.reg_number, v.chassis_number, v.engine_number, v.vehicle_segment, v.purchase_date, v.add_blue_use, v.vehicle_type, v.is_vehicle_movable,v.engine_emmission_type, o.id as ownerId,o.owner_name as owner_name, o.owner_mob, o.owner_landline, o.owner_cat, o.owner_company,o.alse_mail,o.asm_mail,group_concat(oc.id separator ' !! ' ) as ownercontactid,oc.mob as ownercontactmob,oc.owner_contact_email,group_concat(oc.contact_name separator ' !! ' ) as contact_name from mstr_vehicle v left join mstr_owner o on v.ownerId = o.id and o.flag=1 left join mstr_owner_contact oc on o.id=oc.owner_id and oc.flag=1 	where v.reg_number='$reg_number' and v.flag=1");
				$chechVehSql = DB::table('mstr_vehicle')->where('reg_number',$reg_number)->where('flag','1')->count();
			}else if($chassis_number !='000000NA'){
				// $chechVehSql = DB::select("Select v.id as vehicleId, v.vehicle_model, v.reg_number, v.chassis_number, v.engine_number, v.vehicle_segment, v.purchase_date, v.add_blue_use, v.vehicle_type, v.is_vehicle_movable,v.engine_emmission_type, o.id as ownerId,o.owner_name as owner_name, o.owner_mob, o.owner_landline, o.owner_cat, o.owner_company,o.alse_mail,o.asm_mail,group_concat(oc.id separator ' !! ' ) as ownercontactid,oc.mob as ownercontactmob,oc.owner_contact_email,group_concat(oc.contact_name separator ' !! ' ) as contact_name from mstr_vehicle v left join mstr_owner o on v.ownerId = o.id and o.flag=1 left join mstr_owner_contact oc on o.id=oc.owner_id and oc.flag=1 where v.chassis_number = '$chassis_number' and v.flag=1");
				$chechVehSql = DB::table('mstr_vehicle')->where('chassis_number',$chassis_number)->where('flag','1')->count();
			}else if($engine_number !='000000NA'){
				//$chechVehSql = DB::select("Select v.id as vehicleId, v.vehicle_model, v.reg_number, v.chassis_number, v.engine_number, v.vehicle_segment, v.purchase_date, v.add_blue_use, v.vehicle_type, v.is_vehicle_movable,v.engine_emmission_type, o.id as ownerId,o.owner_name as owner_name, o.owner_mob, o.owner_landline, o.owner_cat, o.owner_company,o.alse_mail,o.asm_mail,group_concat(oc.id separator ' !! ' ) as ownercontactid,oc.mob as ownercontactmob,oc.owner_contact_email,group_concat(oc.contact_name separator ' !! ' ) as contact_name from mstr_vehicle v left join mstr_owner o on v.ownerId = o.id and o.flag=1 left join mstr_owner_contact oc on o.id=oc.owner_id and oc.flag=1 	where v.engine_number='$engine_number' and v.flag=1");
				$chechVehSql = DB::table('mstr_vehicle')->where('engine_number',$engine_number)->where('flag','1')->count();
			}
			$chechVehTableSql = DB::table('elitesupport.vahan_details')->where('registration_number',$reg_number)->count();
			// dd($chechVehTableSql);
			if($chechVehTableSql == 0){	
				if($reg_number !='000000NA'){			
				 	$res = $this->getVahanTable($reg_number);
					 if($res == 'tokenError'){
						echo "tokenError";die;
					}		
				}				
			}	
			$checkVehicleQuery = DB::table("elitesupport.vahan_details")->select('registration_number', 'json_req')->where('registration_number',$reg_number)->get();
			//dd($chechVehSql);
			// if(sizeof($chechVehSql) > 0){										
			// if($chechVehSql[0]->reg_number != null){										
			if($chechVehSql){ 
				if(sizeof($checkVehicleQuery)>0){								
					foreach($checkVehicleQuery as $row){
						$jsonReq = json_decode($row->json_req);
						$chassis_number = $jsonReq->vehicle_chasi_number;
						$engine_number = $jsonReq->vehicle_engine_number;
						$vehicle_model = $jsonReq->maker_model;
						$rc_number = $jsonReq->rc_number;
						$engine_emmission_type1 = ($jsonReq->norms_type =='BHARAT STAGE VI')?'BS6':'Non BS6';
						$add_blue_use = $engine_emmission_type1 == 'BS6'?'Yes':'No';
						if(!empty($chassis_number)){
							DB::table('mstr_vehicle')->where('reg_number',$reg_number)->update(['chassis_number'=>$chassis_number,'engine_number'=>$engine_number,'vehicle_model'=>$vehicle_model,'engine_emmission_type'=>$engine_emmission_type1,'add_blue_use'=>$add_blue_use]);
						}						
						$query = DB::select("Select v.id as vehicleId, v.vehicle_model, v.reg_number, v.chassis_number, v.engine_number, v.vehicle_segment, v.purchase_date, v.add_blue_use, v.vehicle_type, v.is_vehicle_movable,v.engine_emmission_type, o.id as ownerId,o.owner_name as owner_name, o.owner_mob, o.owner_landline, o.owner_cat, o.owner_company,o.alse_mail,o.asm_mail,group_concat(oc.id separator ' !! ' ) as ownercontactid,oc.mob as ownercontactmob,oc.owner_contact_email,group_concat(oc.contact_name separator ' !! ' ) as contact_name from mstr_vehicle v left join mstr_owner o on v.ownerId = o.id and o.flag=1 left join mstr_owner_contact oc on o.id=oc.owner_id and oc.flag=1 
						where (v.reg_number=:reg_number or v.chassis_number = :chassis_number or v.engine_number=:engine_number) and v.flag=1",["reg_number"=>$rc_number,"chassis_number"=>$chassis_number,"engine_number"=>$engine_number]);
				...
				if(sizeof($checkVehicleQuery)>0){					
					foreach($checkVehicleQuery as $row){
						$jsonReq = json_decode($row->json_req);
						$chassis_number1 = $jsonReq->vehicle_chasi_number;
						$maker_description = $jsonReq->maker_description;
						$vehicle_category = $jsonReq->vehicle_category;
						// $engine_emmission_type1 = ($jsonReq->norms_type =='BHARAT STAGE VI')?'BS6':'Non BS6';
						$chassis_numberCheck = $chassis_number1 != null?substr($chassis_number1, 0, 3):null;
						$rc_number = $jsonReq->rc_number;
						$vehicle_chasi_number = $jsonReq->vehicle_chasi_number;											
						$vehicle_engine_number = $jsonReq->vehicle_engine_number;
						$maker_model = $jsonReq->maker_model;
						$maker_description = $jsonReq->maker_description;						
						$engine_emmission_type1 = ($jsonReq->norms_type =='BHARAT STAGE VI')?'BS6':'Non BS6';						
						$owner_name = $jsonReq->owner_name;
						$mobile_number = $jsonReq->mobile_number;
						$vehicle_category = $jsonReq->vehicle_category;						
						$add_blue_use = $engine_emmission_type1 == 'BS6'?'Yes':'No';						
						// echo $chassis_number1;die;
						if(($chassis_numberCheck == 'MB1' || $maker_description == 'ASHOK LEYLAND LTD') && $vehicle_category != 'LGV'){ 
							echo 'vahanData'.'##'.$rc_number.'##'.$vehicle_chasi_number.'##'.$vehicle_engine_number.'##'.$maker_model.'##'.$maker_description.'##'.$engine_emmission_type1.'##'.$owner_name.'##'.$mobile_number.'##'.$vehicle_category.'##'.$add_blue_use;die;
						}else if($chassis_numberCheck == 'MB1' && $vehicle_category == 'LGV'){
							echo "lcv";	die;
						}else if($chassis_number1 == null || $chassis_number1 == ''){
							echo "noData";	die;
						} else{
							echo "notcreate";die;
						}
	...
	public function getVahan(Request $request){		
		...
		if(!empty($regNo)){
			try {
				$curl = curl_init(); 
				curl_setopt_array($curl, array(
					// CURLOPT_URL => 'https://mobqty.ashokleyland.com/CustomerPortal/rest/services/getToken',
					// CURLOPT_URL => 'http://10.200.225.138:8080/QtyCPortal/rest/services/getToken', // UAT URL
					//CURLOPT_URL => 'https://mobqty.ashokleyland.com/QtyCPortal/rest/services/getToken',
					CURLOPT_URL => 'http://10.200.225.133:9010/CustomerPortal/rest/services/getToken', // Live URL
					CURLOPT_RETURNTRANSFER => true,
					CURLOPT_ENCODING => '',
					CURLOPT_MAXREDIRS => 10,
					CURLOPT_TIMEOUT => 0,
				...
				curl_setopt_array($curl2, array(
					// CURLOPT_URL => 'https://mobqty.ashokleyland.com/CustomerPortal/rest/services/getVahanDetails',
					// CURLOPT_URL => 'http://10.200.225.138:8080/QtyCPortal/rest/services/getVahanDetails', // UAT URL
					//CURLOPT_URL => 'https://mobqty.ashokleyland.com/QtyCPortal/rest/services/getVahanDetails',
				...
				$chassis_number1 = $dt1->data->vehicle_chasi_number;
				$vehicle_category = $dt1->data->vehicle_category;
				$maker_description = $dt1->data->maker_description;
				if(($vehicle_chasi_number == 'MB1' || $maker_description == 'ASHOK LEYLAND LTD') && $vehicle_category != 'LGV'){ 
					echo json_encode($dt1->data, true); 
				}else if($chassis_number1 == null || $chassis_number1 == ''){
					echo "noData";die;
				}else if($vehicle_chasi_number == 'MB1' && $vehicle_category == 'LGV'){
					echo "lcv";	die;
				} else{
					echo "notcreate";die;
				}
			...
			try{			
				...
				if(($vehicle_chasi_number == 'MB1' || $maker_description == 'ASHOK LEYLAND LTD') && $vehicle_category != 'LGV'){ 
					...
					$query = DB::table('mstr_vehicle')->select('vehicle_model','chassis_number','engine_number')->where('reg_number',$regNo)->get();
					if(sizeof($query)>0){
						$vehicle_model =$dt1->data->maker_model!=''?$dt1->data->maker_model:'';
						$chassis_number = $dt1->data->vehicle_chasi_number !=''?$dt1->data->vehicle_chasi_number:'';
						$engine_number = $dt1->data->vehicle_engine_number !=''?$dt1->data->vehicle_engine_number:'';
						if($vehicle_model != ''){
							DB::table('mstr_vehicle')->where('reg_number',$regNo)->update(['vehicle_model'=>$vehicle_model]);
						}
						if($chassis_number != ''){
							DB::table('mstr_vehicle')->where('reg_number',$regNo)->update(['chassis_number'=>$chassis_number]);
						}
						if($engine_number != ''){
							DB::table('mstr_vehicle')->where('reg_number',$regNo)->update(['engine_number'=>$engine_number]);
						}
					}
				}
					DB::table('elitesupport.vahan_details')->insert(['registration_number'=>$regNo,'json_req'=>$jsonData]);
					die;
				/* } */
	...
	public function getVahanTable($regNo){
		if($regNo != '000000NA'){
			try {			
				$curl = curl_init(); 
				curl_setopt_array($curl, array(
