<div class="row"  id="freshlang">
    <div class="col-md-2" onclick="getFreshForm('{{$type1}}','{{$type2}}','Hindi')">
        <span style="font-size: 10px;"><i class="fa fa-language" aria-hidden="true"></i> Hindi({{$records['Hindi']}})</span>
    </div>
...
<div class="row"  id="freshform">
    <form name="myFresh" method="post" enctype="multipart/form-data" action="{{route('cogent-assign-update')}}">
    ...
    $(document).ready(function(){
      ...
      $("#disposition_fresh").change(function(){
        ...
        if(dispoVal=="Contacted"){
            ...
            var fsubDis = ['Ticket Accepted by WM','Not Accepted- Asked to Reassign','Said- will check and Update'];
            ...
            var fsubDis = ['No Response from Workshop Team','Switch off','Not Reachable','Number Busy'];
            for(let i=0;i<fsubDis.length;i++){
                var newoption = $('<option value="'+fsubDis[i]+'">'+fsubDis[i]+'</option>');
                $('#sub_disposition_fresh').append(newoption);
            }
    ...
    <script>
        function getFreshForm (type1,type2,lang){
                console.log("Call Lang");
                    ...
                    $.ajax({
                        url: "{{route('cogent-assign-lang-ajax')}}",
                        type: 'post',
                        data: {
                        "_token": "{{ csrf_token() }}",
                        type1: type1,
    ...
    $(document).ready(function(){
      $("#disposition_fresh").change(function(){
        ...
        if(dispoVal=="Contacted"){
            ...
            var fsubDis = ['No Response from Workshop Team and Field Team'];
            for(let i=0;i<fsubDis.length;i++){
                var newoption = $('<option value="'+fsubDis[i]+'">'+fsubDis[i]+'</option>');
                $('#sub_disposition_fresh').append(newoption);
            }
