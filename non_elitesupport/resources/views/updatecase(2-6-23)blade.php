...
<style>
            ...
            <div class="card-body">
                ...
                <div class="row" >
                    <div class="col-md-9" style="border: 1px solid #ccc">
						...
						<form name="myForm" method="post" enctype="multipart/form-data" action="{{url('store-update-cases')}}" onsubmit="return formSubmit()">
							...
							<div class="row" >
								...
								<div class="form-group col-md-3">
			                        <label for="datefrom" >Company Name</label>
									<span style="color: red;">*</span>
							...
							<div class="row">
								...
								<div class="form-group col-md-3">
									...
									<input type="tel" name="caller_contact_alt" id="caller_contact_alt" class="form-control" value="{{$caller_contact_alt}}"   readonly/>
			                    </div>
								<div class="form-group col-md-3">
							...
							<div class="row">
								...
								<div class="form-group col-md-6">
									...
									<select name="assign_to" id="assign_to" class="form-control" onchange="getAssignMob(this.value),getAssignWorkManager(this.value,'')"  {{(Auth::user()->role == 29 || Auth::user()->role == 30 || Auth::user()->role == 87 || Auth::user()->role == 78)?"":"disabled"}}>
										<option value="NA">--Select--</option>
										@isset($sqlLatLong)
								...
								<div class="form-group col-md-3">
									...
									@endphp --}}
									<input type="text" name="actual_response_time" id="actual_response_time" autocomplete="off" class="form-control" value="{{$actual_response_time}}" {{($actual_response_time != '')?"readonly":""}} onblur="estimatedChange(this.value)">
								...
								<div class="form-group col-md-3">
									...
									<select name="restoration_type" id="restoration_type" class="form-control" {{$remark_type == 'Closed'?"required":""}} {{(Auth::user()->user_type_id == 3)?"readonly":""}} onchange="restorationType(this.value)">
										<option value="">--Select--</option>
								...
								{{-- @if($actual_response_time != '')
 									...
 									@if($hHours >=4) --}}
 										<div class="form-group col-md-3" id="responsedelayDiv" style="display: none;">
 											...
 											<select name="response_delay_reason" id="response_delay_reason" class="form-control" {{-- {{$remark_type == 'Closed'?"required":""}} {{(Auth::user()->user_type_id == 3)?"readonly":""}} --}}>
 												<option value="">--Select--</option>
								...
								<div class="form-group col-md-3">
									...
									<select name="vehicle_type" id="vehicle_type" class="form-control">
										...
										<option value="NA">--Select--</option>
											<option value="Warranty" {{$vehicle_type =='Warranty'?'selected':''}}>Warranty</option>
											<option value="AMC" {{$vehicle_type =='AMC'?'selected':''}}>AMC</option>
											<option value="Extended Warranty" {{$vehicle_type =='Extended Warranty'?'selected':''}}>Extended Warranty</option>
											<option value="Paid" {{$vehicle_type =='Paid'?'selected':''}}>Paid</option>
											<option value="Self" {{$vehicle_type =='Self'?'selected':''}}>Self</option>
									</select>   
			                    </div>
								<div class="form-group col-md-3">
			                        <label for="Aggregate" >Aggregate</label>
							...
							@php $roleArray = array(29,30,87,78); @endphp
							...
							<br/>
 							...
 							<div class="form-group col-md-12">
 								<p>
 									<span class="btn-primary"  style="padding-left: 5px;padding-right: 5px;padding-top: 2px;padding-bottom: 2px;cursor: pointer;position: relative;top: 13px;" title="{{$complaint_number}} - MSU History" data-toggle="collapse" data-target=".msu" aria-expanded="false" aria-controls="msu_history">{{$complaint_number}} - MSU History</span> 
 								</p>
							...
							<div class="row updateDiv" style="margin-bottom: 10px;">
 ...
 <script>
	/* function restorationType(param){
		if(param == 'Restored by self'){
	...
	function formSubmit(){
 		...
 		if(remark_type=='Closed' || remark_type=='Completed'){ 
			if(vehicle_type == 'NA'){
				toastr.info("Please fill Vehicle type");
				$('#vehicle_type').focus();
				return false;
			}else if(aggregate == 'NA'){
				toastr.info("Please fill aggregate");
				$('#aggregate').focus();
				return false;
			}else{
				return true;
	...
	/* function formSubmit(){
		var createdDateTime = '{{ $createdDate }}';
	 ...
	 function estimatedChange(params) {
		...
		if(estimated_response_time < actual_response_time){			
			...
			toastr.error("Actual Response Time should be greater than Est. reponse time");
		 } */
		/*  var createdDateTime = '{{ $createdDate }}';
		 var actual_response_time = $('#actual_response_time').val();
 		if(actual_response_time !=''){
...
$(document).ready(function () {
		...
		if(latitude!=''){
			...
			//toastr.error("vdrrdrd");
			getAssignDetailsManually();
		}
		
});
	
function addMarker_old(lat,long,el) {
